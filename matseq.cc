...
void create(int n)
{
	...
	clock_t end=clock();
	clock_t prtime=end-start;
	std::cout<<"the procressing time for seq simple multiplication"<<(prtime/CLOCKS_PER_SEC)*1000<<" Milliseconds \n";
...
#pragma omp single
  {
	...
	prtime=end-start;
	std::cout<<"the procressing time for seq strassen part  "<<(prtime/CLOCKS_PER_SEC)*1000<<" Milliseconds \n";
}
}
	/*for(i=0;i<n;i++) {
		std::cout<<"\n";
